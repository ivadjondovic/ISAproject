...
@RequestMapping("api/fishingLessonComplaint")
public class FishingLessonComplaintController {
	
	@Autowired
	private FishingLessonComplaintService complaintService;
	
	@PostMapping(path = "/create")
	@PreAuthorize("hasRole('CLIENT')")
    public ResponseEntity<?> addComplaint(@RequestBody ComplaintDTO dto){
       FishingLessonComplaint complaint = complaintService.addComplaint(dto);
        if(complaint == null) {
